...
-- Hardening (folds in V0's 0049-era definer-RPC revoke). handle_new_auth_user is
-- a TRIGGER function only — it must NOT be callable as a PostgREST RPC by anon /
-- authenticated (it is SECURITY DEFINER and would let a caller insert into
-- public.users). Revoke the default PUBLIC execute grant. The trigger itself runs
