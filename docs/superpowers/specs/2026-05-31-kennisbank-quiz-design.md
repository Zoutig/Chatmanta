...
## 5. Analyse-pijplijn (M2) — category-probe-via-RAG (ontwerp-tournament 2026-05-31)

Het net-nieuwe stuk. Een design-tournament (3 strategieën + judge) koos **category-probe-via-RAG** (8,2/10) boven map-reduce-summarize (6,9 — de oude default) en chunk-budget-truncate (7,0). Reden: het hergebruikt de bestaande `match_chunks` vector-search i.p.v. een niet-bestaande whole-KB-primitive te bouwen, kost **constant ongeacht KB-grootte** (~€0,001 mini / ~€0,01 gpt-4o), kán de silent-truncation-veto niet herintroduceren (er is niets te truncaten), en erft org-isolatie + soft-delete-filter gratis uit de RPC. `analyse_method='map_reduce'` blijft als **A/B-seam** voor een latere diepere variant (migratie-vrij).

**Async job**, getriggerd door Niels vanuit het admin-dashboard:

1. **Trigger** (admin server action, `actionTry → requireV0Auth → requireKnownOrgId`): valideer ≥1 niet-deleted document via `listDocs(orgId)` (anders "Kennisbank is leeg", geen rij). Check-and-block de `UNIQUE(org) WHERE status<>'geannuleerd'` (re-trigger tijdens `concept`/`generating` → 23505). Insert `admin_quiz` in `generating`, keer direct terug. **`maxDuration=120`** op de route (gpt-4o-generatie kan 30–60s; chat's 60 is te krap).
2. **Probes** (background): embed de 8 vaste categorie-probes (Niels' eigen categorie-lijst) in één `embedTexts()`-call; draai 8× `match_chunks(orgId, vec_i, 8)` via `Promise.all`. Score top-1 similarity per categorie: `<0.4` = `ontbreekt`, `0.4–0.55` = `zwak` (beide kandidaat-gap), `≥0.55` = `gedekt` (skip). Branche valt in dezelfde pass (beste chunk seedt `bedrijfscontext`). De 0.4/0.55-banden zijn startwaarden, te valideren (zie §8).
...
> **Productbeslissing (overrijdbaar vóór M2-build):** category-probe vraagt *"zijn de 8 standaard-MKB-categorieën aanwezig?"* — niet *"wat mist uniek voor dít bedrijf?"*. De 8 categorieën zijn Niels' eigen lijst, dus dit is trouw aan het plan. Wil je novel business-specifieke gaps detecteren → map-reduce via de `analyse_method`-seam. **Presence ≠ sufficiency:** een "bel-ons-voor-prijzen"-pagina scoort `gedekt` maar is juist een gap; mitigatie = band-tuning, niet volledig oplosbaar binnen een similarity-only probe.

**Model — BESLIST:** per-klant keuze tussen **`gpt-4o-mini`** (default) en **`gpt-4o`**. Niels kiest het model bij het triggeren van de analyse (model-dropdown in de trigger-UI, §6.1); de keuze wordt opgeslagen in `admin_quiz.analyse_model`. Beide modellen staan al in `MODEL_COSTS_USD` (geen plumbing nodig). Bij `gpt-4o` is de async job (§5) extra belangrijk ivm latency. Claude blijft buiten scope (Anthropic-pad bestaat niet in V0).
