// V1 Klantendashboard — gesprek-detail (transcript).
//
// Read-only. getSessionOrg → getV1Conversation onder de session-client (RLS):
// een thread van een andere org komt niet terug → notFound(). V1 thread_messages
// dragen geen sources-jsonb, dus het transcript is enkel de user/assistant-
// bubbels (geen bronnen-paneel zoals V0).
...
export default async function V1GesprekDetailPage({
  ...
  try {
    ...
    if (isAppError(e) && e.code === 'AUTH_FORBIDDEN') {
      return <PageHead eyebrow="Gesprek" title="Geen toegang" subtitle="Je bent geen lid van deze organisatie." />;
    }
  ...
  // Status uit het laatste assistant-bericht (kind='fallback' = onbeantwoord).
          ...
          >
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: 'var(--klant-muted)', marginTop: 2 }}>
                Voeg kennis toe in de Kennisbank — vergelijkbare vragen worden meteen meegenomen.
              </div>
            </div>
          </div>
        )}
